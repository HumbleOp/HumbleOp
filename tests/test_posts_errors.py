...
def test_comment_on_nonexistent_post(client):
    token = client.post("/register", json={"username": "x3", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    rv = client.post(f"/comment/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"text": "hi"})
    assert rv.status_code == 404

def test_comment_without_text(client):
    token = client.post("/register", json={"username": "x4", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
...
def test_comment_twice_same_user(client):
    token = client.post("/register", json={"username": "x5", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    client.post(f"/create_post/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"body": "B"})
...
def test_eloquent_speaker_badge(client):
    token = client.post("/register", json={"username": "x6", "password": "p"}).get_json()["token"]
    for i in range(20):
        pid = uuid.uuid4().hex
        client.post(f"/create_post/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"body": "B"})
        long_text = "x" * 101
...
def test_vote_invalid_candidate(client):
    token = client.post("/register", json={"username": "x7", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    client.post(f"/create_post/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"body": "B"})
...
def test_vote_twice(client):
    token = client.post("/register", json={"username": "x8", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    client.post(f"/create_post/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"body": "B"})
...
def test_start_duel_with_no_post(client):
    token = client.post("/register", json={"username": "x9", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    rv = client.post(f"/start_duel/{pid}", headers={"Authorization": f"Bearer {token}"})
    assert rv.status_code == 404

def test_start_duel_with_no_comments(client):
    token = client.post("/register", json={"username": "x10", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
...
def test_start_duel_with_no_votes(client):
    token = client.post("/register", json={"username": "x11", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    client.post(f"/create_post/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"body": "text"})
    client.post(f"/comment/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"text": "text"})
    rv = client.post(f"/start_duel/{pid}", headers={"Authorization": f"Bearer {token}"})
...
def test_get_results_with_no_votes(client):
    token = client.post("/register", json={"username": "x12", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    client.post(f"/create_post/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"body": "text"})
    client.post(f"/comment/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"text": "text"})
...
def test_start_now_on_missing_post(client):
    token = client.post("/register", json={"username": "x13", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    rv = client.post(f"/start_now/{pid}", headers={"Authorization": f"Bearer {token}"})
    assert rv.status_code == 404

def test_flag_post_no_winner(client):
    token = client.post("/register", json={"username": "x14", "password": "p"}).get_json()["token"]
    pid = uuid.uuid4().hex
    client.post(f"/create_post/{pid}", headers={"Authorization": f"Bearer {token}"}, json={"body": "B"})
