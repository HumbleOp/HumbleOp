...
def test_get_update_profile(client):
    rv = client.post("/register", json={"username": "a", "password": "p"})
...
def test_follow_unfollow_flow(client):
    # registra due utenti
    ta = client.post("/register", json={"username": "a", "password": "p"}).get_json()["token"]
    tb = client.post("/register", json={"username": "b", "password": "p"}).get_json()["token"]
...
def test_upload_avatar_success(client):
    ...
    rv = client.post("/register", json={"username": "ava", "password": "test"})
    token = rv.get_json()["token"]
...
def test_upload_avatar_missing_file(client):
    rv = client.post("/register", json={"username": "noimg", "password": "p"}).get_json()
    token = rv["token"]
...
def test_upload_avatar_invalid_extension(client):
    rv = client.post("/register", json={"username": "badext", "password": "p"}).get_json()
    token = rv["token"]
...
def test_upload_avatar_empty_filename(client):
    rv = client.post("/register", json={"username": "nofile", "password": "p"}).get_json()
    token = rv["token"]
