def test_insightful_badge(client, unique_post_id, alice_token):
    ...
    token = client.post("/login", json={"username": "xuser", "password": "p"}).json()["access_token"]
    ...
    for i in range(30):
        ...
        voter_token = client.post("/login", json={"username": uname, "password": "p"}).json()["access_token"]
